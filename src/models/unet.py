...
""" Adapted from https://github.com/yiyixuxu/denoising-diffusion-flax
The code is slightly modified to utilize jax.vmap properly
"""
...
def l2norm(t, axis=1, eps=1e-12):
    ...
    denom = jax.lax.stop_gradient(
        jnp.clip(jnp.linalg.norm(t, ord=2, axis=axis, keepdims=True), eps)
    )
    out = t / denom
    return out
...
class SinusoidalPosEmb(nn.Module):
    ...
    def __call__(self, time):
        """
        Args:
          time: jnp.ndarray of shape [1].
        Returns:
          out: embedding vectors with shape `[dim]`
        """
        half_dim = self.dim // 2
...
class Upsample(nn.Module):
    ...
    def __call__(self, x):
        H, W, C = x.shape
        dim = self.dim if self.dim is not None else C
        x = jax.image.resize(x, (H * 2, W * 2, C), "nearest")
...
class ResnetBlock(nn.Module):
    ...
    def __call__(self, x, time_emb):
        """
        Args:
          x: jnp.ndarray of shape [H, W, C]
          time_emb: jnp.ndarray of shape [D]
        Returns:
          x: jnp.ndarray of shape [H, W, C]
        """
        ...
        time_emb = time_emb[None, None, :]  # [H, W, C * 2]
...
class Attention(nn.Module):
    ...
    def __call__(self, x):
        ...
        q, k, v = jnp.split(qkv, 3, axis=-1)  # [H, W, dim]

        q, k, v = map(
            lambda t: rearrange(t, "x y (h d) -> (x y) h d", h=self.heads),
            (q, k, v),
        )
        ...
        sim = jnp.einsum("i h d, j h d -> h i j", q, k) * self.scale
        attn = nn.softmax(sim, axis=-1)
        assert attn.shape == (self.heads, H * W, H * W)

        out = jnp.einsum("h i j , j h d  -> h i d", attn, v)
        out = rearrange(out, "h (x y) d -> x y (h d)", x=H)
        assert out.shape == (H, W, dim)
...
class LinearAttention(nn.Module):
    ...
    def __call__(self, x):
        ...
        out = rearrange(out, "h e (x y) -> x y (h e)", x=H)
        assert out.shape == (H, W, dim)
...
class Unet(nn.Module):
    ...
    def forward(self, x, time):
        ...
        time_emb = SinusoidalPosEmb(self.dim, dtype=self.dtype)(time)  # [1, dim]
    ...
    def __call__(self, x, time):
        return jax.vmap(self.forward)(x, time)
